...
Additional options:
  ...
  - `maxArrayLength`: must be `0`, a positive integer, `Infinity`, or `null`, if present. Default `Infinity`. Specifies the maximum number of elements to include when formatting Arrays, Sets, Maps, and other collections. Elements beyond this limit will be replaced with `... N more items`.
